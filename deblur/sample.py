...
# <codecell>
def build_dataset(n_samps_per_image=10, train_test_split=0.2, gpr_train_size=5000):
...
def sample_residual(residual: np.ndarray, pred_res: np.ndarray) -> np.ndarray:
...
def make_pdf_plot(num_images, train_idxs, pred_res, save_dir):
    with PdfPages(os.path.join(save_dir, "sample_residual_comparison.pdf")) as pdf:
        ...
        for i in tqdm(idxs):
            ...
            fig = _plot_row(i, blur, decoded, residual, curr_pred_res, samp_residual)
            pdf.savefig(fig)
...
for i in tqdm(range(256)):
    idxs = np.floor(decoded_images.flat) == i
    mean = np.mean(residuals.flat[idxs])
    means.append(mean)

# <codecell>
preds, sd = fit_gpr_resid_model(5000, decoded_images, residuals, alpha=5e-5)
pred_res = generate_pred_residuals(decoded_images, preds)

# reg = fit_gpr_resid_model(9000, decoded_images, residuals)
# print("finish train, predicting...")
# pred_res = generate_pred_residuals(decoded_images[0:4], reg)
